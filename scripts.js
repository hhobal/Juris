...
function renderDemoChips() {
  ...
  box.innerHTML = state.advogados.map(a =>
    `<button type="button" class="chip" data-oab="${a.oab}">${a.iniciais} · ${firstName(a.nome)}</button>`
  ).join("");
  box.querySelectorAll(".chip").forEach(btn => {
    btn.addEventListener("click", () => {
      document.getElementById("loginOab").value = btn.dataset.oab;
      document.getElementById("loginCnpj").value = state.empresa.cnpj;
    });
  });
}
...
document.getElementById("loginForm").addEventListener("submit", (e) => {
  e.preventDefault();
  const oab = normalize(document.getElementById("loginOab").value);
  const errorEl = document.getElementById("loginError");
  const advogado = state.advogados.find(a => normalize(a.oab) === oab || normalize(a.oab).replace(/\s/g, "") === oab.replace(/\s/g, ""));
  if (!advogado) {
    errorEl.textContent = "OAB não encontrada no cadastro da empresa. Confira o número ou use um acesso de demonstração.";
    return;
  }
  errorEl.textContent = "";
  doLogin(advogado.id);
...
function prazoRow(p) {
  const proc = getProcesso(p.processoNumero);
  ...
  <div class="task-row" data-goto-page="prazos">
    ...
    <div class="task-row-info">
      ...
      <small>${proc ? proc.parte : p.processoNumero} · ${p.tipo}</small>
    </div>
...
function processoDetailModal(numero) {
  ...
  const adv = getAdvogado(p.advogadoId);
  const prazosDoProc = state.prazos.filter(pr => pr.processoNumero === numero);
...
function prazoCard(p) {
  ...
  <div class="prazo-card ${u.cls}">
    ...
    <div class="prazo-info">
      ...
      <strong>${p.descricao}</strong>
      <small class="mono">${p.processoNumero}</small>
      <small>${proc ? proc.parte : ""}</small>
    </div>
...
function prazoFormModal(prefillProc) {
  return `
  <div class="modal-head">
    <h2>Novo prazo</h2>
    <button class="icon-btn" data-close-modal>✕</button>
  ...
  <form id="prazoForm">
    <label>Descrição
      <input type="text" id="pDescricao" placeholder="Ex: Contestação, Réplica, Audiência..." required>
    </label>
    <label>Processo vinculado
      <select id="pProcesso" required>
        ${state.processos.map(p => `<option value="${p.numero}" ${prefillProc === p.numero ? "selected" : ""}>${p.numero} — ${p.parte}</option>`).join("")}
      </select>
    </label>
    <div class="form-row">
...
function wirePage(page) {
  ...
  if (page === "prazos") {
    ...
    pageContent.querySelectorAll('[data-action="del-prazo"]').forEach(btn => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        state.prazos = state.prazos.filter(p => p.id !== btn.dataset.id);
        saveState();
        loadPage("prazos");
...
function wireModalCommon() {
  ...
  if (addP) addP.addEventListener("click", () => { openModal(prazoFormModal(addP.dataset.proc)); wirePrazoForm(); });
}
...
function wirePrazoForm() {
  ...
  document.getElementById("prazoForm").addEventListener("submit", (e) => {
    ...
    const novo = {
      ...
      processoNumero: document.getElementById("pProcesso").value,
